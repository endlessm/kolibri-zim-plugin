...
MASTER_HASH=$(git ls-remote https://github.com/endlessm/kolibri-zim-plugin.git master | cut -f1)
if [ $HEAD != $MASTER_HASH ]
then
...
TAG_HASH=$(git ls-remote https://github.com/endlessm/kolibri-zim-plugin.git $TAG^{} | cut -f1)
